...
const Incomesortedbyamount = (props) => {
  ...
  useEffect(() => {
    const fetchIncome = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/Fincontrol/income/getIncomeSortedByAmount/${id}`);
  ...
  return (
    <div>
      <h2 className="text-center" style={{color:'cadetblue',marginTop:'20px'}}>INCOME LIST SORTED BY AMOUNT</h2>
      <div className="row">
      </div>
      <Balance></Balance>
      <div className="row">
...
export default Incomesortedbyamount;
